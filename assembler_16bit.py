...
outputbin = os.path.join(dir_path, "output.bin")
...
for var in varibles.keys():
    ...
    output_code[address] = (65280 & value) >> 8
    output_code[address + 1] = 255 & value
...
for label in processed_labels:
    ...
    for i, code_index in zip(range(start_address, len(code) + start_address), range(len(code))):
        output_code[i] = code[code_index]
